...
class EffectsEntry(BaseModel):
    """
    EffectsEntry
    """
    int_minimum: Optional[StrictInt] = Field(None, description="minimum int value, can be a single if ignore_int_max and no ignore_int_min")
    int_maximum: Optional[StrictInt] = Field(None, description="maximum int value, if not ignore_int_max and not ignore_int_min, the effect has a range value")
    type: Optional[EffectsEntryType] = None
    ignore_int_min: Optional[StrictBool] = Field(None, description="ignore the int min field because the actual value is a string. For readability the templated field is the only important field in this case. ")
    ignore_int_max: Optional[StrictBool] = Field(None, description="ignore the int max field, if ignore_int_min is true, int min is a single value")
    formatted: Optional[StrictStr] = Field(None, description="all fields from above encoded in a single string")
    __properties = ["int_minimum", "int_maximum", "type", "ignore_int_min", "ignore_int_max", "formatted"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EffectsEntry:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EffectsEntry:
        """Create an instance of EffectsEntry from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EffectsEntry.parse_obj(obj)

        _obj = EffectsEntry.parse_obj({
            "int_minimum": obj.get("int_minimum"),
            "int_maximum": obj.get("int_maximum"),
            "type": EffectsEntryType.from_dict(obj.get("type")) if obj.get("type") is not None else None,
