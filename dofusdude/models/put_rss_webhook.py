...
class PutRSSWebhook(BaseModel):
    ...
    subscriptions: Optional[conlist(StrictStr, unique_items=True)] = Field(None, description="Get the available subscriptions with /meta/webhooks/rss")
    preview_length: Optional[conint(strict=True, le=4000, ge=0)] = None
    __properties = ["whitelist", "blacklist", "subscriptions", "preview_length"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PutRSSWebhook:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PutRSSWebhook:
        """Create an instance of PutRSSWebhook from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PutRSSWebhook.parse_obj(obj)

        _obj = PutRSSWebhook.parse_obj({
